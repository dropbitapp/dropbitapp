...
export default {
  name: 'AddFermentableView',
  components: {
    DbitTable,
  },
  created() {
    this.$store.dispatch('purchase/getRawMaterialsForPurchase', 'Fermentable');
  ...
  computed: {
    ...
    batchVolumeWeightMessage() {
      return (this.batchVolume && !this.batchWeight) || (this.batchWeight && !this.batchVolume) ? '' : 'Either Batch Volume or Batch Weight field is required.';
    },
  },
  data() {
    return {
    ...
    submit() {
      ...
      this.$store
        ...
        .then(() => {
          ...
          this.$toast.open({
            duration: 5000,
            message: `Successfully created fermentable: ${
        ...
        .catch(() => {
          this.$toast.open({
            duration: 5000,
            message: `Unable to create fermentable: ${purchase.PurBatchName}`,
            type: 'is-danger',
          });
        });
    },
