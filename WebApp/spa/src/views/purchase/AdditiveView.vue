<template>
  <div class="container">
    <div class="box">
      <dbit-table :data="additives" rowId="PurchaseId">
...
export default {
  ...
  created() {
    this.$store.dispatch('purchase/getPurchases', 'Additive');
  },
  computed: {
    additives() {
      // TODO: Refactor || empty object array
      if (this.$store.state.purchase.additives) {
        return this.$store.state.purchase.additives;
      }
      return [{}];
    },
  },
    ...
    deleteItem(purchase) {
      this.$store
        ...
        .then(() => {
          this.$toast.open({
            duration: 5000,
            message: `Successfully deleted additive purchase: ${
        ...
        .catch(() => {
          this.$toast.open({
            duration: 5000,
            message: `Unable to delete additive purchase: ${
              purchase.PurBatchName
            }`,
            type: 'is-danger',
          });
