...
export default {
  name: 'AddDistilledView',
  components: {
    DbitTable,
  },
  created() {
    this.$store.dispatch('purchase/getRawMaterialsForPurchase', 'Distilled');
    ...
    submit() {
      ...
      this.$store
        ...
        .then(() => {
          ...
          this.$toast.open({
            duration: 5000,
            message: `Successfully created distilled: ${purchase.PurBatchName}`,
            type: 'is-success',
          });
        })
        .catch(() => {
          this.$toast.open({
            duration: 5000,
            message: `Unable to create distilled: ${purchase.PurBatchName}`,
            type: 'is-danger',
          });
        });
    },
