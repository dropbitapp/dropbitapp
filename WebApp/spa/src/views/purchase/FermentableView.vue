<template>
  <div class="container">
    <div class="box">
      <dbit-table :data="fermentables" rowId="PurchaseId">
...
export default {
  name: 'FermentableView',
  components: {
    DbitTable,
  },
  created() {
    this.$store.dispatch('purchase/getPurchases', 'Fermentable');
  },
  computed: {
    fermentables() {
      // TODO: Refactor || empty object array
      if (this.$store.state.purchase.fermentables) {
        return this.$store.state.purchase.fermentables;
      }
      return [{}];
    },
  },
    ...
    deleteItem(purchase) {
      this.$store
        .dispatch('purchase/deletePurchase', {
          workflow: 'Fermentable',
          id: purchase.PurchaseId,
        })
        .then(() => {
          this.$toast.open({
            duration: 5000,
            message: `Successfully deleted fermentable purchase: ${
        ...
        .catch(() => {
          this.$toast.open({
            duration: 5000,
            message: `Unable to delete fermentable purchase: ${
              purchase.PurBatchName
            }`,
            type: 'is-danger',
          });
