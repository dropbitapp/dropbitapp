...
export default {
  ...
  created() {
    this.$store.dispatch('purchase/getRawMaterialsForPurchase', 'Supply');
    ...
    submit() {
      ...
      this.$store
        ...
        .then(() => {
          ...
          this.$toast.open({
            duration: 5000,
            message: `Successfully created supply: ${
        ...
        .catch(() => {
          this.$toast.open({
            duration: 5000,
            message: `Unable to create supply: ${purchase.PurBatchName}`,
            type: 'is-danger',
          });
        });
    },
