<template>
  <div class="container">
    <div class="box">
      <dbit-table :data="fermentation" rowId="ProductionId">
...
export default {
  ...
  created() {
    this.$store.dispatch('production/getProductions', 'Fermentation');
  },
  computed: {
    fermentation() {
      // TODO: Refactor || empty object array
      if (this.$store.state.production.fermentations) {
        return this.$store.state.production.fermentations;
      }
      return [{}];
    },
  },
    ...
    deleteItem(production) {
      this.$store
        ...
        .then(() => {
          this.$toast.open({
            duration: 5000,
            message: `Successfully deleted fermentation: ${
        ...
        .catch(() => {
          this.$toast.open({
            duration: 5000,
            message: `Unable to delete fermentation: ${
              production.BatchName
            }`,
            type: 'is-danger',
          });
